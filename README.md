...
# Basic Workflow: Environment setup & Pipeline Functions
...
**Output:**
- substructure_vector_dict: dict, mapping from observed glycans (keys) to a ```substructure_present_absent_vector``` (e.g. [match_dict.json](https://github.com/LewisLabUCSD/GlyCompare/blob/master/example_data/paper_epo/intermediate_file/paper_epo_match_dict.json)). The ```substructure_present_absent_vector``` indicates the number of times each substructure occurs in a glycan. The i-th substructure referenced in the ```substructure_present_absent_vector``` matches the i-th substructure in the ```motif_vec``` which contains the glycoct for each extracted and merged substructure. The ```motif_vec``` is returned by ```glycan_io.motif_dict_to_motif_vec(substructure_vector_dict)```. 
...
- simple_naming: boolean, defaults to False. If False, glycan structure will be read from the glytoucan database using m/z or glytoucan IDs from the abundance table. If True, the glycan identifiers specified in the abundance table will be copied directly. ```simple_naming``` is only relevant when ```simple_profile=True```.
- external_profile_naming: boolean, defaults to False. If False, ```glycoprofile_pip``` will ignore glycoprofile/sample names. If True, ``` external_profile_naming.json``` will be read to name the glycoprofiles/samples.
...
- keywords_dict: dict, keyword pipeline variable dictionary from ```load_para_keywords``` function
- linkage_specific: boolean, if True, ```extract_and_merge_substructures_pip``` will leverage linkage information. If False, ```extract_and_merge_substructures_pip``` will run without linkage information. Do not set ```linkage_specification=True``` if this information is not supported by your measurement, this will result in unpredictable behavior. 
- epitope: boolean, if True, ```select_motifs_pip``` will examine all substructures. If False, ```select_motifs_pip``` will only examine substrutures orginating at the root.
