...
- **glycan_id**
  ...
  - type: str. string ID used for a glycan.
- **glycan_str**

  - type: str. IUPAC structure of a glycan. Generated by str(glycan).
- **mz**

  - type: str. Annotated m/z for a glycan

- **motif(substructure)**

  - type: a glycan object. A substructure of a glycan
- **motif_id** 

  - type: str. string index ID used for a glycan.
- **motif_vec**

  - type: list (of glycan). A list of glycan
- **motif_dict**
...
- **match_vec**: 
  ...
  - type: list (of float). length = len(motif_vec), The existency(0/1/1+) within a glycan for each motif in motif_vec
- **match_mtrix**: 

  - type: 2D list (of float). row size: number of glycan, column size: len(motif_vec), a list of match_vec
...
- **glycan_profile_obj**: type: class
  ...
  .mz_list: 
  - type: list. A list of m/z for each glycan.
  .weighted_vec: 
  - as mentioned
  .relative_abundance: 
  - as mentioned
