...
# support functions
##################################
def get_children(m,mh,dir='child'):
	if dir=='child':
		return mh[mh[:,0]==m,1]
	elif dir=='parent':
		return mh[mh[:,1]==m,0]
...
# Occurance based
def hier_enrich_glycoprofile_occurence(glycoprofile,scoredMotifs_occurence_vector,motif_hierarchy,motif_vector,method='chi_squared',relative='child'):
...
def enrich_glycoprofile(glycoprofiles,condition,scoredMotifs_abundance_matrix,motif_hierarchy,motif_vector,method='anova',relative='child'):
	...
	methods -- 'anova' does not consider the hierachy, direct comparison of motif association with condition. 'wald' wald test on motif of interest while accounting for variance better described by a parent or child (as specificed by the 'relative' parameter),'LRT' likelihood ratio test on motif of interest while removing variance better described by a parent or child (as specificed by the 'relative' parameter)
	...
	for motif in motif_vector:
		...
		for child in children:
			...
			df = pd.DataFrame(condition=condition,child=c,motif=m)
